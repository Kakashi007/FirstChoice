...
listOrder = async () => {
   ...
   try{
     ...
     console.log("No of order: " + order.data.listOrders.items.length)
     this.setState({number:order.data.listOrders.items.length})
     console.log(this.state.queryResponse);
   } catch(err) {
     console.log(err);listOrder
    }
 }
      ...
      <Container>
       <Header style={[{  height: 90, borderBottomColor: '#757575' }, styles.androidHeader]}>
                    <Left style={{ flexDirection: 'row' }}>
                     ...
                     <Title style={{marginTop:18,marginRight:20}}> CANCELLED ORDERS </Title>
                    <Right>
        ...
        <Footer>
          <FooterTab>
            <Button badge vertical onPress={() => { this.props.navigation.navigate('Delivered') }} >
            ...
            <Button active badge vertical onPress={() => { this.props.navigation.navigate('Cancelled') }} >
              <Badge ><Text>{this.state.number}</Text></Badge>
